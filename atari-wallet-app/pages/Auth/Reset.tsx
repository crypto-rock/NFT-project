...
const Reset = ({navigation} : any) => {
	...
	const submit = async () => {
		if (validateEmail(status.email) === false) {
			updateStatus({errmsg : 'Invalid email'})
		...
		<Layout>
			<View style = {styles.middle}> 
				<View style = {styles.backpanel}>
				 	<Image source = {imgPass}  style = {{ ...theme.mlauto, ...theme.mrauto, ...theme.mt1, resizeMode:'contain', width:50}}/> 
					<Text style = {styles.title}>Forget password</Text>
					<Text style = {{ ...theme.lightcolor, ...theme.textcenter,  ...theme.t, ...theme.mt3}}>We just need your registred email to send you password reset instruections</Text>
					{status.success ? (
