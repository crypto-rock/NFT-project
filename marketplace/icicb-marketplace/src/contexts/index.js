...
import {
    ...
    marketPlaceContract, supportChainId, multiCallContract,
    NFTContractABI, supportedNFTs
} from "../contracts"
import { delay, toBigNum, fromBigNum } from "../utils/utils"
...
export default function Provider({ children }) {
    ...
    const checkApproval = async (to) => {
        try {
            if (wallet.status === "connected") {
                ...
                const allowance = await signedAtariContract.allowance(wallet.account, to);
    ...
    const updateTokenBalance = async () => {
        ...
        var allowance = checkApproval(marketPlaceContract.address);
    ...
    const getNFTContract = (NFTaddress) => {
        return new ethers.Contract(NFTaddress, NFTContractABI, providers[supportChainId])
    }
    ...
    const checkNFTApproval = async (NFTaddress, id) => {
        try {
            if (wallet.status === "connected") {
                ...
                return allowance.toUpperCase() === marketPlaceContract.address.toUpperCase();
            } else {
                return false;
            }
        } catch (err) {
    ...
    const updateNFTContents = async (NFTaddress) => {
        try {
            ...
            if (ids !== []) {
                var _tokenInfos = await multiCallContract.getNFTInfos(NFTaddress, ids);
                ...
                try {
                    var _tokenPriceInfos = await NFTContract.getPriceInfos(ids);

                    ids.map((id) => {
                        tokenInfos[id] = {
                            ...tokenInfos[id],
                            price: String(_tokenPriceInfos[id])
                        }
                    })
                ...
                for (var id in tokenInfos) {
                    if ((tokenInfos[id].owner).toUpperCase() == (marketPlaceContract.address).toUpperCase())
                        marketIds.push(id);
                }

                if (marketIds.length != 0) await updateMarketContents(NFTaddress, marketIds);
            ...
            NotificationManager.error("updateNFT error :", err.message);
        }
    }
    ...
    const updateMarketContents = async (NFTaddress, ids) => {
        try {
            ...
            var _marketOrderInfos = await marketPlaceContract.getOrderByAssetIds(NFTaddress, ids);
            var _marketBidInfos = await marketPlaceContract.getBidByAssetIds(NFTaddress, ids);
            var marketInfos = {};
    ...
    const approveAtari = async (to, amount) => {
        try {
            if (wallet.status !== "connected") throw new Error("Please connect wallet.")
            const signedTokenContract = atariContract.connect(state.signer);
            var tx = await signedTokenContract.approve(to, String(amount));
            await tx.wait();
            updateTokenBalance();
    ...
    const buyBaseNFT = async (NFTaddress, id) => {
        try {
            ...
            var tx = await signedNFTContract.buy(id);
            await tx.wait();
    ...
    const buyNFT = async (NFTaddress, id, price) => {
        try {
            ...
            var tx = await signedMarketPlaceContract.Buy(NFTaddress, id, toBigNum(price, 0))
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const approveNFT = async (NFTaddress, id) => {
        try {
            const signedNFTContract = getSignedNFTContract(NFTaddress);
            var tx = await signedNFTContract.approve(marketPlaceContract.address, id);
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const onSaleNFT = async (NFTaddress, id, price, endTime) => {
        try {
            const signedMarketPlaceContract = marketPlaceContract.connect(state.signer);
            var tx = await signedMarketPlaceContract.createOrder(NFTaddress, id, toBigNum(price, 0), endTime);
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const cancelOrder = async (NFTaddress, id) => {
        try {
            const signedMarketPlaceContract = marketPlaceContract.connect(state.signer);
            var tx = await signedMarketPlaceContract.cancelOrder(NFTaddress, id);
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const bidNFT = async (NFTaddress, id, price, endTime) => {
        try {
            const signedMarketPlaceContract = marketPlaceContract.connect(state.signer);
            var tx = await signedMarketPlaceContract.PlaceBid(NFTaddress, id, toBigNum(price, 0), endTime)
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const acceptBid = async (NFTaddress, id, price) => {
        try {
            const signedMarketPlaceContract = marketPlaceContract.connect(state.signer);
            var tx = await signedMarketPlaceContract.acceptBid(NFTaddress, id, price)
            await tx.wait();
            updateAll();
            updateTokenBalance();
    ...
    const GetMarketNFTs = () => {
        try {
            ...
            var marketNFTs = Object.fromEntries(
                Object.entries(NFTDATAS).filter(([key, NFTdata]) =>
                    NFTdata.owner.toUpperCase() === marketPlaceContract.address.toUpperCase() ||
                    NFTdata.owner.toUpperCase() === state.supportedNFTs[0].toUpperCase()
                )
            )
            return marketNFTs;
        } catch (err) {
    ...
    const GetUserNFTs = () => {
        try {
            if (wallet.status != "connected") throw new Error("connect wallet!");
            var NFTDATAS = state.NFTDATAS[state.supportedNFTs[0]];
            var userNFTs = Object.fromEntries(
                Object.entries(NFTDATAS).filter(([key, NFTdata]) =>
                    NFTdata.owner.toUpperCase() === wallet.account.toUpperCase()
                )
            )
            return userNFTs;
        } catch (err) {
    ...
    const GetOnsaledUserNFTs = () => {
        try {
            ...
            var userOnsaledNFTs = Object.fromEntries(
                Object.entries(MARKETDATAS).filter(([key, Marketdata]) =>
                    Marketdata.orders.seller.toUpperCase() === wallet.account.toUpperCase()
                )
            )
